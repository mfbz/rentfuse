...
export class Nep17Contract {
	...
	public async balanceOf(address: string): Promise<number> {
		...
		try {
			const response = await this.rpcClient.invokeFunction(this.contractHash.toString(), 'balanceOf', [
	 ...
	 */
	public async transfer(from: string, to: string, amount: number): Promise<string> {
		...
		if (this.config.account === undefined || this.config.account.address != from) {
