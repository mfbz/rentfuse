...
export async function calculateNetworkFee(
	...
	hashes.map((hash) => {
		...
		if (witnessScript === undefined)
			...
			networkFeeSize +=
				u.getSerializedSize(invocationScriptSize) + invocationScriptSize + u.getSerializedSize(witnessScript);
			networkFee += execFeeFactor * sc.OpCodePrices[sc.OpCode.PUSHDATA1] * signatureCount;
			...
			networkFee += execFeeFactor * sc.OpCodePrices[pushOpcode];
			networkFee +=
				execFeeFactor * (sc.getInteropServicePrice(sc.InteropServiceCode.SYSTEM_CRYPTO_CHECKMULTISIG) * publicKeyCount);
		}
	...
	try {
		...
		const nativeContractPolicyFeePerByte = parseInt(response.stack[0].value as string);
...
export async function setBlockExpiry(
	...
	if (blocksTillExpiry && !(blocksTillExpiry > tx.Transaction.MAX_TRANSACTION_LIFESPAN))
		blockLifeSpan = blocksTillExpiry;
	const rpcClient = new rpc.RPCClient(config.rpcAddress);
	transaction.validUntilBlock = (await rpcClient.getBlockCount()) + blockLifeSpan - 1;
}
 ...
 */
export async function addFees(transaction: tx.Transaction, config: CommonConfig): Promise<void> {
	...
	if (config.networkFeeOverride) {
		...
		transaction.networkFee = await calculateNetworkFee(transaction, config.account, config);
	}
 ...
 */
export function getContractHash(sender: u.HexString, nefChecksum: number, contractName: string): string {
